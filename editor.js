 ...
 */
    ...
    jsEditor.on("inputRead", function (editor, change) {
        ...
        if (change.text[0] === "" && change.from.ch > 0 && change.from.line === cursor.line) {
    ...
    function validateHtml(html) {
        ...
        Array.from(doc.body.childNodes).forEach((node) => {
            if (node.nodeType === Node.ELEMENT_NODE) {
                ...
                if (!selfClosingTags.includes(tagName) && !html.includes(`</${tagName}>`)) {
    ...
    function validateCss(css) {
        ...
        const validProperties = [
            ...
            "background-color", "background-image", "border", "border-radius", "border-color",
            "border-style", "border-width", "box-shadow", "color", "display", "flex",
            "flex-direction", "flex-wrap", "font", "font-size", "font-weight", "grid",
            "grid-template-columns", "grid-template-rows", "height", "margin", "padding",
            "position", "text-align", "text-decoration", "width", "z-index", "opacity",
            "overflow", "visibility", "transition", "transform", "white-space", "line-height",
            "letter-spacing", "word-spacing", "background-size", "background-repeat",
            "border-collapse", "border-spacing", "list-style", "outline", "overflow-x",
            "overflow-y", "text-transform", "vertical-align", "text-shadow", "filter",
            "cursor", "pointer-events", "clip", "min-width", "max-width", "min-height",
            "max-height", "object-fit", "object-position", "resize", "text-overflow",
            "box-sizing", "font-family", "margin-bottom","justify-content","margin-top",
        ];
    ...
    function validateJavaScript(js) {
        ...
        if (openParens > 0) {
            errors.push({ line: lines.length, message: "Unmatched opening parentheses." });
        }
        if (openBrackets > 0) {
            errors.push({ line: lines.length, message: "Unmatched opening brackets." });
        }
    ...
    function displayErrorIcons(htmlErrors, cssErrors, jsErrors) {
        document.getElementById("html-error-icon").style.display = htmlErrors.length > 0 ? "inline" : "none";
        document.getElementById("css-error-icon").style.display = cssErrors.length > 0 ? "inline" : "none";
        document.getElementById("js-error-icon").style.display = jsErrors.length > 0 ? "inline" : "none";

        if (htmlErrors.length > 0) {
            htmlErrors.forEach(err => {
                accumulatedErrors.push(`HTML Error on line ${err.line}: ${err.message}`);
            });
        }
        ...
        if (jsErrors.length > 0) {
            jsErrors.forEach(err => {
                accumulatedErrors.push(`JavaScript Error on line ${err.line}: ${err.message}`);
            });
        }
    }

    document.getElementById("speak-errors").addEventListener("click", function () {
    ...
    window.run = function () {
        ...
        if (htmlErrors.length === 0 && cssErrors.length === 0 && jsErrors.length === 0) {
